...
int bsps_battery_vol_adc_init(void);
void bsps_battery_adc_task(void);

OS_INIT_REGISTER("battery_vol_adc_init", bsps_battery_vol_adc_init, 0, 5);
OS_TSK_REGISTER(bsps_battery_adc_task, PRIORITY_4, 1000);
...
void bsps_battery_adc_task(void)
{
	...
	uint16_t vol = HAL_ADC_GetValue(&hadc2);
	float bat_vol = vol * 3.3f * 2.0f / 65536.0f;
	bsps_ui_battery_vol_draw(bat_vol);
}
